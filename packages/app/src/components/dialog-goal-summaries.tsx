  ...
  return (
    <div class="flex flex-col gap-2 rounded-md border border-border-subtle p-3">
      <div class="flex items-center justify-between gap-2">
        <span class="text-13-medium" classList={{ "text-text-strong": !!props.latest, "text-text-base": !props.latest }}>
          {props.latest
      ...
      <Show when={props.summary.created}>
        {(created) => (
          <span class="text-12-regular text-text-weak">{new Date(created()).toLocaleString()}</span>
        )}
      </Show>
      <For each={parsed()}>
        {(section) => (
