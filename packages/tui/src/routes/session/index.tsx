...
export function Session() {
  ...
  const hasStickyQueuedPrompts = createMemo(() => {
    return steerQueuedMessages().length > 0 || lifecycleQueued().length > 0
  })
...
function DialogBtwResume(props: { parentSessionID: string }) {
  ...
  return (
    <DialogSelect
      ...
      onSelect={(option) => {
        dialog.replace(() => (
          <DialogBtw parentSessionID={props.parentSessionID} sessionID={option.value === "new" ? undefined : option.value} />
        ))
      }}
    />
  )
          ...
          >
            <box height={Math.max(10, dimensions().height - 10)} minHeight={10}>
              ...
              <box flexShrink={0}>
                ...
                <Show when={permissions().length === 0 && questions().length > 0}>
                  <QuestionPrompt request={questions()[0]} directory={sync.session.get(questions()[0].sessionID)?.directory} />
                </Show>
...
function StickyQueuedMessage(props: { text: string; badge: "steer" | "queue"; color: RGBA; index: number; shell?: boolean }) {
  const { theme } = useTheme()
...
function LifecycleQueuedMessage(props: { item: LifecycleQueuedPrompt; index: number }) {
  ...
  return (
    <StickyQueuedMessage text={text()} badge="queue" color={theme.secondary} index={props.index} shell={props.item.mode === "shell"} />
  )
}
