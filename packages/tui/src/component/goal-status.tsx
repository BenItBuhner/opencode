...
import type { GoalSummariesView, GoalSummaryView } from "./dialog-goal-summaries"
...
export function goalDetailsMessage(goal: GoalStatusView, elapsed: string | undefined) {
  return [`Status: ${goal.status}`, elapsed ? `Running: ${elapsed}` : undefined, "", goal.text]
...
export function GoalSidebarStatus(props: {
  ...
  const fadedAccent = createMemo(() =>
    RGBA.fromValues(theme.accent.r, theme.accent.g, theme.accent.b, theme.accent.a * 0.45),
  )
