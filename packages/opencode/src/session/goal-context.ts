import { remainingTokens, type Info } from "./goal"

export function goalContext(goal: Info) {
  const prompt = goal.status === "budget_limited" ? budgetLimitPrompt(goal) : continuationPrompt(goal)
  return `<goal_context>\n${prompt}\n</goal_context>`
}

function budget(goal: Info) {
  return [
...
function continuationPrompt(goal: Info) {
...
function budgetLimitPrompt(goal: Info) {
