...
export function minimumLogLevel() {
  const value = (process.env.OPENGOAL_LOG_LEVEL ?? process.env.OPENCODE_LOG_LEVEL)?.toUpperCase()
  const levels = {
    DEBUG: "Debug",
    INFO: "Info",
    WARN: "Warn",
...
export function loggers() {
  return process.env.OPENGOAL_PRINT_LOGS === "1" || process.env.OPENCODE_PRINT_LOGS === "1"
